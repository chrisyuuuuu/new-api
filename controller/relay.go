...
func logConsumerErrLog(c *gin.Context, relayErr *dto.OpenAIErrorWithStatusCode) {
	channelId := c.GetInt("channel_id")
	errMessage, _ := json.Marshal(relayErr.Error)
	monitorChannelId := os.Getenv("MONITORING_CHANNEL_ID")
	tokenName := c.GetString("token_name")
	if strconv.Itoa(channelId) == monitorChannelId {
		//	飞书消息发送
		requestPath := "http://" + fmt.Sprintf("%s%s", c.Request.Host, c.Request.URL.Path)
		requestBody, _ := common.GetRequestBody(c)
		requestBodyUnescape, _ := UnescapeUnicode(string(requestBody))
		currentTime := time.Now()
		formattedTime := currentTime.Format("2006-01-02 15:04:05")
		cli := client.NewFeiShuClient("", "")
		errMessageUnescape, _ := UnescapeUnicode(string(errMessage))
		elements := message.InteractiveMessageCardElements{
			{
				Tag: "div",
				Text: message.InteractiveMessageCardElementsText{
					Content: fmt.Sprintf("**告警时间**：%v\n**渠道ID**：**210007**\n**RequestID**：%s\n**接口地址**：%s\n**业务来源**：%s\n**请求参数**：\n%v\n**问题详情**：\n%v\n<at id=all></at>",
						formattedTime, c.GetString("X-Oneapi-Request-Id"), requestPath, tokenName, requestBodyUnescape, errMessageUnescape),
					Tag: "lark_md",
				},
			},
		}
		header := message.InteractiveMessageCardHeader{
			Title: message.InteractiveMessageCardHeaderTitle{
				Content: "OneAPI监控告警",
				Tag:     "plain_text",
			},
			Template: "blue",
		}
		err := cli.SendInteractiveMessage(elements, header)
		if err != nil {
			logger.Error(c, fmt.Sprintf("飞书消息报警推送失败,error:%v", err))
		}
	}
}
