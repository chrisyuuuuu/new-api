...
type Log struct {
	...
	AlphaUserId      string `json:"alpha_user_id" gorm:"default:NULL;index"`
	AlphaUserName    string `json:"alpha_user_name" gorm:"default:NULL;index"`
	...
	LogErr           string `json:"log_error" gorm:"type:varchar(1500);default:''"`
}
...
func RecordConsumeLog(ctx context.Context, userId int, alphaUserId, alphaUsername string, channelId int, promptTokens int, completionTokens int, modelName string, tokenName string, quota int, content string, tokenId int, userQuota int, useTimeSeconds int, isStream bool, other map[string]interface{}) {
	common.LogInfo(ctx, fmt.Sprintf("record consume log: userId=%d,alphaUserId=%s,alphaUsername=%s, 用户调用前余额=%d, channelId=%d, promptTokens=%d, completionTokens=%d, modelName=%s, tokenName=%s, quota=%d, content=%s, logErr=%s", userId, alphaUserId, alphaUsername, userQuota, channelId, promptTokens, completionTokens, modelName, tokenName, quota, content))
